...
// Usage:
//   node backend/sync_google_sheets.js --sheetId=<ID> --gid=<GID> [--type=Project|CR|auto]
...
if (!SHEET_ID || !GID) {
  console.error('Usage: node backend/sync_google_sheets.js --sheetId=<ID> --gid=<GID> [--type=Project|CR|auto]');
...
function importFromRows(rows, data) {
  ...
  for (const raw of rows) {
    ...
    const milestone = pick(n, ['milestone'], '');
    ...
    if (type === 'CR') {
      ...
      const existingCR = data.changeRequests.find(c => c.initiativeId === id);
      if (existingCR) Object.assign(existingCR, cr); else data.changeRequests.push(cr);
      addedCRs += 1;
    }
  }
